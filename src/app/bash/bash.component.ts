...
export class BashComponent implements AfterViewInit, OnInit {
  ...
  fileOperations = [
    ...
    { key: 'find ./ -name "*.exe" -type f -print0', command: 'Will start Find in the current directory, will search by name for files that match the "*.exe" search string (notice that surrounding a word with * will find all results that include that word). Directories will not be shown because it is searching for files only, with -type f. The print0 argument tells Find to not consider whitespace as the end of a file name.' },
    { key: 'find . -type f -printf "%s %p\n" |sort -nr|head', command: 'Will return a list with the 10 largest files in a directory and its subdirectories.' },
